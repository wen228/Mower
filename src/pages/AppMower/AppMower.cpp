...
void AppMower::Update() {
    ...
    const float tgt_rpm = (float)s.target_raw / 100.0f;
    if (!s.ready) {
        ...
        snprintf(telem, sizeof(telem),
                 "%s\nRPM:%.0f  TGT:%.0f\nI:%.1f  Vin:%.2f  P:%.2fW\n"
                 "Load:%s  SOC:%.0f%%",
                 tip, (double)s.speed, (double)tgt_rpm, (double)s.current,
                 (double)s.vin, (double)s.batt_power_w, Mower::loadName(s.load),
                 (double)s.batt_soc_pct);
    } else {
        snprintf(telem, sizeof(telem),
                 "RPM:%.0f  TGT:%.0f\nI:%.1f  Vin:%.2f  P:%.2fW\n"
                 "Load:%s  SOC:%.0f%%",
                 (double)s.speed, (double)tgt_rpm, (double)s.current,
                 (double)s.vin, (double)s.batt_power_w, Mower::loadName(s.load),
                 (double)s.batt_soc_pct);
    }
